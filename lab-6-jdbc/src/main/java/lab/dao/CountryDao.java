...
public class CountryDao extends JdbcDaoSupport {
	...
	public void loadCountries() {
		for (String[] countryData : COUNTRY_INIT_DATA) {
			getJdbcTemplate().update(LOAD_COUNTRIES_SQL, (Object[]) countryData);
		}
	}

	public Country getCountryByCodeName(String codeName) {
		return getJdbcTemplate().query(GET_COUNTRY_BY_CODE_NAME_SQL, new String[] {codeName}, COUNTRY_ROW_MAPPER).get(0);
	}

	public Country getCountryByName(String name)
			throws CountryNotFoundException {
		JdbcTemplate jdbcTemplate = getJdbcTemplate();
		List<Country> countryList = jdbcTemplate.query(GET_COUNTRY_BY_NAME_SQL, new String[] {name}, COUNTRY_ROW_MAPPER);
